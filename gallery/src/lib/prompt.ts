...
export function buildSplitPrompt(entry: Entry, ctx: PromptContext): string {
  ...
  const lines: (string | null)[] = [
    ...
    "```bash",
    'npx instant-elements element new <조각이름> --intent "…" --summary "…" --category <아래 기준> --keywords "…"',
    "```",
    "- 조각도 갤러리에 뜨는 정식 엔트리다. 임시 파일로 흩뿌리지 않는다.",
    "- **조각의 카테고리는 원본과 다를 수 있다.** 그 자체로 완결된 UI 가 아니라 여러 컴포넌트가",
    "  공유하는 **구조·리듬**이면 `System` 이다(예: 여러 이력 목록이 같이 쓰는 타임라인 뼈대).",
    "  화면에 놓이는 완성형이면 `Composite`, 움직임이면 `Animations`.",
    "- 데모도 함께 채운다 — 조각이 갤러리 카드에서 안 보이면 다음 사람이 그 존재를 모른다.",
    "",
    "### 4. 재조립",
    ...
    `  ${meta.category === "Composite" ? "원본은 이미 Composite 이니 category 는 그대로 둔다." : "조각을 조립하는 자리가 되었으므로 meta.category 를 Composite 로 맞춘다."}`,
    "- props 를 뺐거나 바꿨으면 스키마도 함께 백필한다(아래 명령이 처리한다).",
    "- 인덱스와 검증을 다시 돌린다:",
    "```bash",
    "npx instant-elements index && npx instant-elements element schema && npx instant-elements element validate",
    "```",
    "",
    "### 6. 기록",
    "코드 변경을 먼저 커밋한 뒤:",
    "```bash",
    `npx instant-elements element log ${entry.name} --action modified \\`,
    '  --note "<N>개 조각으로 분할" \\',
    "  --sha \"$(git rev-parse HEAD)\"",
    "```",
    "",
