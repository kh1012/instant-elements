...
export function buildModifyPrompt(
  ...
  const lines: (string | null)[] = [
    ...
    "```bash",
    `${CLI} element log ${entry.name} --action modified \\`,
    '  --note "<무엇을 바꿨는지 한 줄>" \\',
    '  --prompt "<위 요청사항 원문>" \\',
...
export function buildSplitPrompt(entry: Entry, ctx: PromptContext): string {
  ...
  const lines: (string | null)[] = [
    ...
    "```bash",
    `${CLI} element new <조각이름> --intent "…" --summary "…" --category <아래 기준> --keywords "…"`,
    "```",
    ...
    "- 인덱스와 검증을 다시 돌린다:",
    "```bash",
    `${CLI} index && ${CLI} element schema && ${CLI} element validate`,
    "```",
    "",
    "### 6. 기록",
    "코드 변경을 먼저 커밋한 뒤:",
...
export function buildCreatePrompt(request: string, ctx: PromptContext): string {
  const lines = [
    ...
    "",
    "## 순서",
    `1. **먼저 찾는다.** \`${CLI} element list\` 로 비슷한 역할이 이미 있는지 본다. 있으면 새로 만들지 말고`,
